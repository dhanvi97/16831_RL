...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        if self.nn_baseline:
            ...
            q_normalized = normalize(q_values, np.mean(q_values), np.std(q_values))
            q_normalized = ptu.from_numpy(q_normalized)
            
            self.baseline_optimizer.zero_grad()
            pred_mean = self.baseline(observations).squeeze()
            baseline_loss = self.baseline_loss(pred_mean, q_normalized)
